...
fun GeneralTabContent(
    ...
    SettingsGroup() {
        ...
        if (config.workshopMods) {
            SettingsSwitch(
                colors = settingsTileColorsAlt(),
                title = { Text(text = stringResource(R.string.workshop_local_only)) },
                subtitle = { Text(text = stringResource(R.string.workshop_local_only_description)) },
                state = config.workshopLocalOnly,
                onCheckedChange = { state.config.value = config.copy(workshopLocalOnly = it) },
            )
        }
